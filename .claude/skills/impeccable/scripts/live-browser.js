...
(function () {
    ...
    const LIVE_UI_SURFACES = Array.isArray(
    ...
    function syncConfigureInputChrome() {
        ...
        const surface =
            barEl?.dataset.configureSurface === "true" ? barEl : null;
    ...
    function groupSiblingRows(siblings, rowThreshold) {
        ...
        for (const entry of sorted) {
            ...
            for (const row of rows) {
                if (
                    Math.abs(entry.rect.top - row[0].rect.top) <= rowThreshold
                ) {
                    row.push(entry);
                    placed = true;
                    break;
    ...
    function buildInsertConfigureRow() {
        ...
        row.appendChild(inputShell);
        row.appendChild(
            buildConfigureTrailingCluster([count], voiceBtn, create)
        );
    ...
    function buildCyclingRow() {
        ...
        if (arrivedVariants === 0) {
            ...
            accept.title =
                "Accept becomes available when the first variant arrives";
        }
    ...
    function toggleActionPicker() {
        ...
        const P =
            pickerEl.__iceq_palette || barPaletteForTheme(detectPageTheme());
    ...
    function buildParamsPanel(variantEl, params) {
        ...
        for (const p of params) {
            ...
            if (p.kind === "range") {
                ...
                const activeOpt =
                    opts.find(o => o.value === p.default) || opts[0];
    ...
    function restoreMixedTextNodeManualEdit(op) {
        ...
        if (
            byIndex &&
            normalizeManualContextText(byIndex.nodeValue) === newText
        ) {
    ...
    function mountedParameterCount() {
        if (svelteComponentSession?.sessionId === currentSessionId) {
            return Object.values(
                ...
                (total, params) =>
                    total + (Array.isArray(params) ? params.length : 0),
                0
            );
        }
        const wrapper = findVariantsWrapper(currentSessionId);
    ...
    function parseOriginalMarkupElement(originalMarkup) {
        ...
        return (
            doc.getElementById("impeccable-anchor")?.firstElementChild || null
        );
    }
    ...
    async function mountSvelteComponentVariant(variantNum) {
        ...
        const dirAbs =
            manifest.revisionDirAbs || manifest.componentDirAbs || null;
        const moduleBase =
    ...
    function reportVariantMounted(sessionId, variantNum, moduleUrl) {
        ...
        sendEvent({
            ...
            variant,
            url: moduleUrl
                ? String(moduleUrl).slice(0, MOUNT_URL_MAX)
                : undefined
        });
    }
    ...
    function sourceHasSessionWrapper(text, sessionId) {
        ...
        return (
            src.indexOf('data-impeccable-variants="' + sessionId + '"') !==
                -1 ||
            src.indexOf("data-impeccable-variants='" + sessionId + "'") !==
                -1 ||
    ...
    function injectVariantsFromSource(filePath, sessionId, opts = {}) {
        ...
        if (isJsxSourceFile(filePath)) {
            ...
            if (
                ...
                completeSourceInjection(liveWrapper, sessionId, {
                    ...opts,
                    filePath
                });
                return;
        ...
        fetch(url)
            ...
            .then(html => {
                    ...
                    if (opts.orphanDiscard && sessionId === currentSessionId) {
                        ...
                        if (attempt < COMPLETED_SOURCE_FALLBACK_RETRIES) {
                            ...
                            discardOrphanedSession(
                                "variant wrapper missing from source"
                            );
                        }
                    }
                    return;
                ...
                if (existingWrapper) {
                    ...
                    completeSourceInjection(wrapper, sessionId, {
                        ...opts,
                        filePath
                    });
                    return;
        ...
        const obs = new MutationObserver(mutations => {
            ...
            if (selectedElement && !document.body.contains(selectedElement)) {
                ...
                if (isInsert) {
                    const visEl =
                        count > 0
                            ? pickVariantContent(wrapper, visibleVariant || 1)
                            : null;
                    if (visEl) {
                    ...
                    selectedElement =
                        pickVariantContent(wrapper, "original") || wrapper;
                }
            ...
            switch (msg.type) {
                ...
                case "agent_phase":
                    if (
                        ...
                        updateBarContent(
                            state === "CYCLING" ? "cycling" : "generating"
                        );
                        saveSession();
                    }
                    break;
        ...
        await Promise.all(
            [...urls].map(async url => {
                try {
                    ...
                    const ext =
                        url.toLowerCase().match(FONT_EXT_RE)?.[1] || "woff2";
                    const mime = FONT_MIME[ext] || "application/octet-stream";
                    map.set(
                        url,
                        "data:" + mime + ";base64," + bufferToBase64(buf)
                    );
                } catch {
    ...
    function shouldUseAncestorCropShaderProxy(el) {
        ...
        return isFrameworkComponentPreviewMode(
    ...
    async function captureElementToBlob(el, snapshot, rect) {
        ...
        const hasAnnotations =
            snapshot &&
            (snapshot.comments.length > 0 || snapshot.strokes.length > 0);
        let annotNode = null;
        ...
        try {
            ...
            if (bg !== "#ffffff") {
                ...
                return {
                    blob,
                    paper: bg ? cssColorToRgb01(bg) : resolvePaperRgb(el)
                };
            }
...
void main() {
  ...
  gl_FragColor = vec4(mix(ground, u_accent, dotAmt), tex.a);
    ...
    async function handleAccept() {
        ...
        sendEvent(acceptPayload, { throwOnError: true })
            .then(() => {
                ...
                document.documentElement.dataset.impeccableAcceptToPickingMs =
                    String(Date.now() - acceptPayload.clientSentAt);
    ...
    function acceptedDomAlreadyClean(pending) {
        ...
        const matches = [
            ...document.querySelectorAll(pending.acceptedSelector)
        ];
        return (
            matches.length > 0 &&
            matches.every(
        ...
        if (svelteComponentSession?.sessionId === cleanupSessionId) {
            ...
            setTimeout(function () {
                ...
                if (hasFrameworkHmrOwnership(lateWrapper)) {
                    ...
                    setTimeout(function () {
                        const staleWrappers =
                            discardedWrappers(cleanupSessionId);
                        if (
    ...
    function clearHandledWrapperReloadStamp(sessionId) {
        try {
            if (sessionId) {
                ...
                const legacy =
                    sessionStorage.getItem(HANDLED_WRAPPER_RELOAD_KEY) || "";
                if (
                    legacy === sessionId ||
                    legacy.startsWith(sessionId + ":")
                ) {
    ...
    function scheduleHandledRuntimeWrapperReload(
        ...
        try {
            reloadAttempts =
                Number(
                    sessionStorage.getItem(handledWrapperReloadKey(sessionId))
                ) || 0;
    ...
    function watchForHandledRuntimeWrapper(
        ...
        const timer = setTimeout(function () {
            if (
                handledRuntimeWrapperWatchers.get(sessionId)?.observer !==
                observer
            )
                return;
            observer.disconnect();
    ...
    function restoreSessionSupersedingHandledWrapper(runtimeWrapper) {
        ...
        return restoreSessionWithoutWrapper(
            "browser_resumed_over_handled_wrapper"
        );
    }
    ...
    function resumeSession(
        ...
        if (restoreSessionSupersedingHandledWrapper(runtimeWrapper))
            return true;
        if (
            scheduleHandledRuntimeWrapperReload(
                runtimeWrapper,
                recoveryRevision
            )
        )
            ...
            visibleVariant =
                ...
                svelteComponentSession.mountedVariant <= arrivedVariants
                    ...
                    : savedVisibleVariant > 0 &&
                      ...
                      : 1;
            selectedElement =
                resolveSvelteComponentAnchor() || wrapper.parentElement;
            setLiveState("CYCLING");
            hideShaderOverlay();
            showBar("cycling");
            startScrollTracking();
    ...
    const AGENT_BUSY_TIP =
        "Agent is working - steering resumes when it finishes";
    ...
    function syncPageChatChrome() {
        ...
        const inputFocused =
            pageChatInput && activeElementDeep() === pageChatInput;
    ...
    function syncPageChatSendButton() {
        ...
        const visible =
            !steerLocked && (pageChatExpanded || hasText || focused);
    ...
    async function fetchDesignSystem() {
        ...
        try {
            ...
            designState.error =
                jsonData.parseError || jsonData.sidecarError || null;
        } catch (err) {
    ...
    function buildTypographyModels(fmTypography, typographyMeta) {
        ...
        return Object.entries(fmTypography).map(([key, spec]) => {
            ...
            return {
                ...
                lineHeight:
                    spec?.lineHeight != null ? String(spec.lineHeight) : "",
    ...
    function buildRadiiModels(fmRounded) {
        if (!fmRounded) return [];
        return Object.entries(fmRounded).map(([name, value]) => ({
            name,
            value
        }));
    }
    ...
    function findProseDescription(proseColors, key, displayName) {
        ...
        for (const g of proseColors.groups) {
            for (const c of g.colors || []) {
                ...
                if (
                    hay &&
                    needles.some(n => hay.includes(n) || n.includes(hay))
                ) {
