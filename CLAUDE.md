...
pnpm db:types       # regenerate src/lib/db/database.types.ts from local Supabase
...
- **No `any`, no `as` casts** except at the Supabase boundary, and there only inside `src/lib/db/`. Parse with Zod on the way out of the DB.
...
- **`src/domain/` imports nothing.** No React, no Supabase, no Next. It is pure functions and schemas. If you need to reach for a dependency there, stop and ask.
...
- TanStack Query for client cache only. Query keys come from the factory in `src/lib/query-keys.ts` — never inline an array literal.
- Forms: react-hook-form + `zodResolver`, schema imported from `src/domain/`.
- Charts: Recharts via the shadcn `chart` component. Theme colours only, no hex literals.
- IDs are branded types (`SurveyId`, `QuestionId`, `ResponseId`). Construct via the helpers in `src/domain/ids.ts`.
...
- For `src/domain/`, write the test first, then the implementation.
