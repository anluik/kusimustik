...
## 034 — The survey's own words are content: a header block, translated and autosaved with the document

**Status:** accepted

**Context.** 030 gave `surveys.elements` its locale-keyed shape and left the survey's own `title` and `description` as plain columns, arguing they name the survey in the owner's list, in their tab and in the CSV's filename. That was true and it was not the whole truth: the runner renders both to the *respondent* — the title in the sticky header and in `<title>`, the description above the first question — so once 033 gave respondents a language picker, an Estonian survey read in Russian was Russian everywhere except those two places, on every screen. 030 said a translated survey name would be "a separate decision, not a consequence of this one"; PLAN's Phase 12 named it the obvious next thing. This is it.

The second half was forced by the first. The two fields lived in the builder's **settings dialog**, which covers the app bar — and the app bar is where the language switcher is. Translating them there would have needed a second switcher, inside a modal, for two fields. That is the tell that they were in the wrong place: they are not settings, they are the first thing a respondent reads.

**Decisions.**

- **They are locale-keyed, and they stay columns.** `surveys.title` and `surveys.description` become `jsonb`, and so do `survey_versions`'. They are not moved into `elements`: the title is still the survey's identity for the owner's list, their tab, the CSV filename and the wave-comparison label, and a magic first element would have broken `sync_survey_questions` (002), the survey list and the create dialog at once. What changed is the shape of the words, not where they live.

  The migration is `alter column ... type jsonb using ...`, **not** 20260909120000's disable-the-triggers-and-UPDATE. That pattern existed because rewriting *values* fires `surveys_before_write()` and `sync_survey_questions()`; a *type* change is DDL and fires no row-level trigger at all, so it bumps no version and snapshots nothing, with no window in which the triggers are off. `surveys_before_write()` needed no change either — its `is distinct from` works on jsonb, and gets slightly better, since jsonb normalises key order and re-saving the same translations in a different order is therefore not an edit.

- **They are the survey's *header block*: the first row of the element list, above question 1.** Selectable like an element, drawn as the first card on the canvas, edited in the editor panel. That is the whole of what it took to give them the translation surface 031 already built — the app-bar switcher, the empty-field-with-the-source-sentence-as-placeholder projection, the caution mark on the row, and the publish dialog's count. No new mechanism, and no second switcher.

  It has no drag handle, cannot be deleted and cannot be duplicated. It is deliberately **out of the `elements` array and out of `SortableContext.items`**: the reorder handlers index into that array, so a row prepended to it would land every drop one position out. It is also hoisted out of the list's empty-survey branch, which used to replace the whole `DndContext` — a survey with no questions yet now shows its own title rather than nothing.

- **`TranslationProvider` stops knowing what an element is.** It takes a `ReadonlyMap<TextPath, LocalizedText>` rather than an `AuthoredElement`, because `"title"` and `"description"` were already `TextPath`s and a head and an element are therefore addressed identically. `referenceTexts` and `missingTranslations` were re-keyed on the same map, so there is one body between the two shapes rather than a second copy of the fallback rule. All nine editors are untouched.

- **The head joins the elements in one debounced save.** `saveSurveyElementsAction` became `saveSurveyDocumentAction` and carries `{ title, description, elements }` under one version token, one dirty state, one save indicator. A separate save for the title would have been a second thing that can fail on a surface whose whole promise is that it saves itself. The autosave's hold-back gate grew to match: a title emptied in *every* language is the empty map, which the schema refuses and the save waits on — while a title merely untranslated in the language on screen parses and goes out, because holding the save for the duration of a translation is exactly the bug 031 warned about.

- **The settings dialog keeps what is genuinely settings**: the language the survey is written in, the languages it is offered in, and the wave label. Three things *about* the survey rather than three things it says.

- **The rename dialog is deleted, along with `renameSurveyAction`.** This is the decision with the sharpest edge. A one-field dialog on the survey list could only ever write one language, and writing the column outright would have replaced every translation of the title with a single string — silently, from a menu item labelled "rename". The alternative was to merge carefully in that one action and hope nobody ever adds a second such writer. Removing it instead leaves **exactly one** writer of the survey's title, the header block, so the mistake has nowhere to happen. `duplicateSurveyAction` is unaffected: it copies the stored document whole, and `NewSurvey.title` is the locale-keyed shape precisely so that a survey written in three languages is duplicated in three.

- **Only the runner resolves through the respondent's language.** Every owner surface — the list, both tabs, the CSV filename, the wave labels — resolves through the survey's *own* locale, via `resolveSurveyTitle`. The translation exists for the respondent; the survey's name in the owner's index does not shift when they switch the app's language. `toSummary` therefore resolves inside the repository, which makes it the **second** documented exception to 030's "the repository returns the stored document" — beside `getRunnerSurveyBySlug`, and for the mirror-image reason: every consumer of a summary is an owner surface and every one of them wants the same single answer.

- **The builder always has something selected.** `initialDocument` selects the head, and a removal falls back to it, so `null` left the selection union entirely and the editor panel's empty state went with it. Every survey has a head; a panel with nothing in it is no longer a state the builder can reach.
